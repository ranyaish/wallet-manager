...
// עוזר קטן כדי לא לחזור על עצמנו
...
export async function fetchBalances(search) {
  ...
  if (search && search.trim()) q = q.ilike("full_name", `%${search}%`);
...
export async function topup({ customer_id, amount, method, reference, notes, operator = "Ran", executed_at }) {
  const { error } = await wallet().from("ledger").insert([{
    customer_id, kind: "topup", amount, method, reference: reference || null, notes: notes || null, operator,
...
export async function redeem({ customer_id, amount, reference, notes, operator = "Ran", executed_at }) {
  const { error } = await wallet().from("ledger").insert([{
    customer_id, kind: "redeem", amount, reference: reference || null, notes: notes || null, operator,
    executed_at: executed_at || new Date().toISOString()
  }]);
  if (error) throw error;
}
...
export async function createCustomer({ full_name, phone, email, notes }) {
  const { error } = await wallet().from("customers").insert([{ full_name, phone, email, notes }]);
  if (error) throw error;
}
